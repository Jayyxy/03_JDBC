...
import java.sql.Statement; //java.sql꺼 import
...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
	   ...
	   // Connection 객체가 잘 생성되엇는지 확인 (객체 주소 반환)
			System.out.println(conn); //oracle.jdbc.driver.T4CConnection@6973bf95
			
			/* SQL 작성 유의사항 */
			// JDBC 코드에서 SQL작성시 세미콜론(;) 작성 하면 안된다 (문자열 내에)
			...
			String sql = "SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, SALARY, HIRE_DATE "
						+ "FROM EMPLOYEE";
			// 문자열 내에 공백을 안넣으면 오류 발생 
			// ex) HIRE_DATEFROM으로 인식
			...
			// -> sql이 DML(INSERT, UPDATE, DELECT) 일 때 실행 메서드 
			// -> 결과로 int 반환 (삽입, 수정, 삭제된 행의 갯수)
			rs = stmt.executeQuery(sql); 
			
			/* 6. 조회 결과가 담겨있는 ResultSet 을 
			 * 	  커서 (cursor)를 이용해서
			 * 	  1행씩 접근해 각 행에 작성된 컬럼 값 가져오기  */
			
			
			// boolean ResultSet.nect() :
			// 커서를 다음 행으로 이동 시킨 후
			...
			/* 7. 사용완료된 JDBC 객체 차원 JDBC 객체 자원 반환 (close) */ 
			// -> 자원반환을 하지 않으면 DB와 연결된 Connection이 그대로 남아 있어서
			// 다른 클라이언트가(ex. Java 프로그램) 추가적으로 연결되지 못하는 문제가 발생될 수 있다
