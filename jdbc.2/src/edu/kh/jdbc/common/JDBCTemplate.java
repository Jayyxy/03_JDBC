...
public class JDBCTemplate {
	...
	public static Connection getConnection() {
		...
		// 2. Properties가 제공하는 메서드를 이용해서 drive.xml 파일 내용을 읽어오기
		
		String filePath = "driver.xml";
		
		prop.loadFromXML(new FileInputStream(filePath));
		
		// 3. prop에 저장된값을 이용해서 Connection 객체 생ㅅ어
...
/*	전달 받은 Statement or PreparedStatment 둘 다 close() 하는 메서드
 * 	+ 다형성의 업캐스팅 적용
...
public static void close(ResultSet rs) {
	...
	try {
		...
		System.out.println("Result close() 중 예외 발생");
		e.printStackTrace();
	}
}